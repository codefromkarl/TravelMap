import { agent, currentLang, showToast, PROVIDER_MODELS, PROVIDER_REGISTRY, getAmapKey, getTestedProviders, addTestedProvider, removeTestedProvider, TESTED_PROVIDERS_KEY, getAvailableProviders, isFreeProvider, requiresApiKey, getPiProvider, getDefaultModel } from '../infra/context.js';
...
function updateProviderOptions() {
  ...
  provSelect.innerHTML = availableProviders.map(p => 
    `<option value="${p}">${PROVIDER_REGISTRY[p]?.displayName || p}</option>`
  ).join('');
...
async function testApiKey(provider, apiKey) {
  ...
  try {
    ...
    const model = getModel(piProvider, testModel);
