...
- Never deploy or run browser automation.
...
def build_pi_command(skill_path: str, prompt: str) -> list[str]:
    return ["pi", "--skill", skill_path, "-p", prompt]


# ── Tick / Loop ──────────────────────────────────────────────────────────────

def tick(repo_root: Path, *, dry_run: bool, cooldown_s: float,
         max_runs: int | None, skill_path: str, now: float) -> str:
    ...
    cmd = build_pi_command(skill_path, prompt)
    ...
    try:
        print(f"[idle-explorer] launching pi ...")
