...
describe("Provider Adapter 单元测试", () => {
  describe("Rnote", () => {
    it("应正确构造请求并解析响应", async () => {
      ...
      const reviews = await fetchRnote("故宫", {
        token: "test-token",
        baseUrl: "https://rnote.test",
      });
    ...
    it("错误码应抛出异常", async () => {
      ...
      await expect(
        fetchRnote("故宫", { token: "bad", baseUrl: "https://rnote.test" }),
      ).rejects.toThrow("Rnote code: 401");
    });
  });
  ...
  describe("JustOneAPI", () => {
    it("应正确构造请求并解析响应", async () => {
      ...
      const reviews = await fetchJustOneApi("长城", {
        token: "test-token",
        baseUrl: "https://justone.test",
      });
      expect(reviews).toHaveLength(1);
  ...
  describe("TikHub", () => {
    it("应正确构造请求并解析响应", async () => {
      ...
      const reviews = await fetchTikHub("西湖", {
        token: "tik-token",
        baseUrl: "https://tikhub.test",
      });
      expect(reviews).toHaveLength(1);
